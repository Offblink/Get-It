...
精致的桌面日程提醒工具。指定时间弹出游戏风提醒窗口，支持延后、自定义铃声、角色装扮。

## 快速开始

```bash
pip install PyQt6 pygame numpy Pillow
python get_it.py              # 正常启动
python get_it.py --minimized  # 启动后最小化到托盘
```

**单文件即用** — `get_it.py` 包含全部功能，无其他依赖文件。`daily_reminder_data.json` 首次运行自动创建。
...
| 提醒弹窗 | 右下角弹出，淡入淡出动画，角色头像 + 名称 |
| 延后 (Snooze) | 延后 N 分钟再次提醒，**不改变原始日程时间** |
| 内置铃声 | 7 种程序化合成铃声 |
| 自定义铃声 | mp3 / wav / ogg |
| 角色装扮 | 名称、颜色、字体、圆形头像（内置裁剪工具） |
| 系统托盘 | 关闭最小化到托盘，双击恢复 |
| 休眠补提醒 | 电脑休眠/关闭期间错过的日程，恢复后自动补提醒（可开关） |
| 数据兼容 | JSON 格式，与旧版 tkinter 版数据互通 |
...
|---|---|---|
| 点击"延后" | `schedule.hour/minute` 被 `now + N` 永久覆盖 | 仅设 `schedule.snoozed_until`，原始时间不变 |
| 到期后 | 日程时间已变，无法恢复 | `snoozed_until` 到期后自动恢复原始时间触发 |

## 休眠补提醒

设置 → "应用休眠/关闭期间，补提醒错过的日程"。

实现：提醒线程检测到检查间隔 > 120 秒（系统休眠或应用未运行）时，自动遍历所有日程补触发间隙内错过的提醒。
